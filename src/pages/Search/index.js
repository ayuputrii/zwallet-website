        ...
        <div className="content-main search-content">
          ...
          <div className="input px-3 px-sm-0">
            ...
            <input
              name="q"
              type="search"
              onChange={(e) => dispatch(searchByName(token, e.target.value))}
              autoComplete="off"
          ...
          <div className="d-flex flex-column ml-3 d-sm-none">
            ...
            <p style={{ color: "#8F8F8F" }} className="small">
              {data.filter((item) => item.phone).length} Contact Found
            </p>
          </div>
          <div style={{ overflowY: "scroll" }} className="list">
            {data.map((item, index) => {
              if (item.phone) {
                return (
                  <Link
                    onClick={() => dispatch(getUserTransfer(token, item.phone))}
                    to={{
